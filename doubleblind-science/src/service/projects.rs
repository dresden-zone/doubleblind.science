...
impl ProjectService {
    ...
    if let Some(user) = user::Entity::find_by_id(owner_uuid).one(&*self.db).await? {
      Ok(Some(
        project::ActiveModel {
          ...
          commit: Set(commit.to_string()),
          github_name: Set(Some(github_name.to_string())),
          created_at: Set(OffsetDateTime::now_utc()),
          last_update: Set(OffsetDateTime::now_utc()),
          github_webhook_secret: Set(Some(secret_token.to_string())),
  ...
  pub(crate) async fn trust_project(&self, project_id: Uuid) -> anyhow::Result<bool> {
    ...
    if let Some(project) = found_project {
      project::ActiveModel {
        ...
        github_name: Unchanged(project.github_name),
  ...
  pub(crate) async fn get_projects_by_github_id(
    ...
    Ok(
      project::Entity::find()
        .filter(project::Column::GithubName.eq(github_name))
        .one(&*self.db)
        .await?,
    )
  }
      ...
      project::Entity::find()
        .filter(
          Condition::any()
            ...
            .add(project::Column::GithubName.eq(repo_name)),
        )
        .one(&*self.db)
        .await?,
    )
