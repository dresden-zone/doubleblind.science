...
pub(super) async fn user_repos(
  ...
  if let (Some(mut access_token), Some(access_token_expr), Some(refresh_token), Some(refresh_token_expr)) = (user_info.github_access_token, user_info.github_access_token_expire, user_info.github_refresh_token, user_info.github_refresh_token_expire) {

    if access_token_expr > OffsetDateTime::now_utc() {
      match state.user_service.fresh_access_token(&mut state.oauth_github_client, session.user_id).await {
        Some(new_token) => {
...
pub(super) async fn create_project(
  ...
  match state.project_service.get_project_by_name_or_repo(data.name, data.repo).await {
